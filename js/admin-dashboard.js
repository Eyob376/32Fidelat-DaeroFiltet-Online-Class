...
async function refreshApplicantsCache() {
    ...
    allApplicantsCache = (data || []).map(row => {
        ...
        return {
            ...
            assignedTeacher: normalizeAssignedTeacher(row.assigned_teacher),
    ...
    const viewerTeacher = getDashboardViewerAssignedTeacher();
    if (viewerTeacher) {
        allApplicantsCache = allApplicantsCache.filter((row) => row.assignedTeacher === viewerTeacher);
    }
}
...
function getDashboardViewerAssignedTeacher() {
    const role = String(sessionStorage.getItem("daeroUserRole") || "").trim().toLowerCase();
    if (role !== "teacher") return "";
    return normalizeAssignedTeacher(sessionStorage.getItem("daeroAssignedTeacher"));
}

const DASHBOARD_PERMISSION_SETTING_KEY = "dashboardPermissions";
const DASHBOARD_PERMISSION_USER_OPTIONS = ["asier", "eyob", "itdev", "admin"];
let dashboardPermissionConfig = {};

function normalizePermissionUser(value) {
    const user = String(value || "").trim().toLowerCase();
    if (DASHBOARD_PERMISSION_USER_OPTIONS.includes(user)) return user;
    return "asier";
}

function getCurrentDashboardPermissionUser() {
    const role = String(sessionStorage.getItem("daeroUserRole") || "").trim().toLowerCase();
    if (role === "teacher") {
        return normalizePermissionUser(sessionStorage.getItem("daeroAssignedTeacher"));
    }
    if (role === "itdev") return "itdev";
    return "admin";
}

function getSidebarLinkItemsForPermissions() {
    const links = Array.from(document.querySelectorAll(".admin-link[data-target]"));
    return links
        .map((link) => {
            const target = String(link.dataset.target || "").trim();
            if (!target || target === "controlPanelPage") return null;
            return { target, label: String(link.textContent || target).trim() };
        })
        .filter(Boolean);
}

function ensureUserPermissionDefaults(user) {
    const normalizedUser = normalizePermissionUser(user);
    if (!dashboardPermissionConfig[normalizedUser] || typeof dashboardPermissionConfig[normalizedUser] !== "object") {
        dashboardPermissionConfig[normalizedUser] = {};
    }

    const entries = getSidebarLinkItemsForPermissions();
    entries.forEach((item) => {
        if (typeof dashboardPermissionConfig[normalizedUser][item.target] !== "boolean") {
            dashboardPermissionConfig[normalizedUser][item.target] = true;
        }
    });
}

async function loadDashboardPermissionConfig() {
    if (typeof db === "undefined" || !db.appSettings?.get) {
        dashboardPermissionConfig = {};
        return;
    }

    try {
        const { data } = await db.appSettings.get(DASHBOARD_PERMISSION_SETTING_KEY);
        const parsed = data?.value ? JSON.parse(data.value) : {};
        dashboardPermissionConfig = (parsed && typeof parsed === "object") ? parsed : {};
    } catch (e) {
        dashboardPermissionConfig = {};
    }

    DASHBOARD_PERMISSION_USER_OPTIONS.forEach((user) => ensureUserPermissionDefaults(user));
}

async function saveDashboardPermissionConfig() {
    if (typeof db === "undefined" || !db.appSettings?.set) return;
    await db.appSettings.set(DASHBOARD_PERMISSION_SETTING_KEY, JSON.stringify(dashboardPermissionConfig));
}

function canCurrentUserAccessTarget(target) {
    const cleanTarget = String(target || "").trim();
    if (!cleanTarget) return false;

    const currentUser = getCurrentDashboardPermissionUser();
    if (currentUser === "admin") return true;
    if (cleanTarget === "controlPanelPage") return currentUser === "itdev";

    ensureUserPermissionDefaults(currentUser);
    return !!dashboardPermissionConfig[currentUser][cleanTarget];
}

function applySidebarLinkPermissions() {
    const currentUser = getCurrentDashboardPermissionUser();
    const links = Array.from(document.querySelectorAll(".admin-link[data-target]"));
    ...
    links.forEach((link) => {
        ...
        if (target === "controlPanelPage") {
            const canManage = currentUser === "admin" || currentUser === "itdev";
            link.style.display = canManage ? "" : "none";
            return;
        }

        link.style.display = canCurrentUserAccessTarget(target) ? "" : "none";
    });
}

function setPermissionStatus(message, ok) {
...
function renderPermissionChecklist() {
    const listEl = document.getElementById("permissionList");
    const userSelect = document.getElementById("permissionUserSelect");
    if (!listEl || !userSelect) return;

    const selectedUser = normalizePermissionUser(userSelect.value);
    ensureUserPermissionDefaults(selectedUser);

    if (selectedUser === "admin") {
        listEl.innerHTML = `<div class="message-item">Admin always has full access. Permissions are not restricted for Admin.</div>`;
        return;
    }

    const rows = getSidebarLinkItemsForPermissions();
    listEl.innerHTML = rows.map((item) => {
        const checked = dashboardPermissionConfig[selectedUser][item.target] ? "checked" : "";
        return `
            <label class="message-item" style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                <input type="checkbox" class="permission-toggle" data-user="${selectedUser}" data-target="${item.target}" ${checked}>
                <span>${item.label}</span>
            </label>
        `;
    }).join("");

    listEl.querySelectorAll(".permission-toggle").forEach((checkbox) => {
        checkbox.addEventListener("change", async () => {
            const user = normalizePermissionUser(checkbox.dataset.user);
            const target = String(checkbox.dataset.target || "").trim();
            if (!target) return;

            ensureUserPermissionDefaults(user);
            dashboardPermissionConfig[user][target] = checkbox.checked;
            await saveDashboardPermissionConfig();
            applySidebarLinkPermissions();
            setPermissionStatus(`Permission updated for ${user} on ${target}.`, true);
        });
    });
}

function initControlPanelPermissions() {
    const userSelect = document.getElementById("permissionUserSelect");
    const enableAllBtn = document.getElementById("permissionSelectAllBtn");
    const disableAllBtn = document.getElementById("permissionClearAllBtn");
    const reloadBtn = document.getElementById("permissionReloadBtn");
    ...
    enableAllBtn?.addEventListener("click", async () => {
        const selectedUser = normalizePermissionUser(userSelect.value);
        if (selectedUser === "admin") {
            setPermissionStatus("Admin already has full access.", true);
            return;
        }

        ensureUserPermissionDefaults(selectedUser);
        getSidebarLinkItemsForPermissions().forEach((item) => {
            dashboardPermissionConfig[selectedUser][item.target] = true;
        });

        await saveDashboardPermissionConfig();
        renderPermissionChecklist();
        applySidebarLinkPermissions();
        setPermissionStatus(`All permissions enabled for ${selectedUser}.`, true);
    });

    disableAllBtn?.addEventListener("click", async () => {
        const selectedUser = normalizePermissionUser(userSelect.value);
        if (selectedUser === "admin") {
            setPermissionStatus("Admin cannot be restricted.", false);
            return;
        }

        ensureUserPermissionDefaults(selectedUser);
        getSidebarLinkItemsForPermissions().forEach((item) => {
            dashboardPermissionConfig[selectedUser][item.target] = false;
        });

        await saveDashboardPermissionConfig();
        renderPermissionChecklist();
        applySidebarLinkPermissions();
        setPermissionStatus(`All permissions disabled for ${selectedUser}.`, true);
    });

    reloadBtn?.addEventListener("click", async () => {
        await loadDashboardPermissionConfig();
        applySidebarLinkPermissions();
        renderPermissionChecklist();
        setPermissionStatus("Permissions reloaded.", true);
    });
...
function exportToCsv(data, filename) {
    const header = ["Guardian Name","Email","Phone","Country","City","Student","Start Date","Grade Level","Assigned Teacher","Status","Year"];
...
function exportToXlsx(data, filename) {
    ...
    const rows = [
        ["Guardian Name","Email","Phone","Country","City","Student","Start Date","Grade Level","Assigned Teacher","Status","Year"]
    ];
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const isTeacher = existingRole === "teacher";
    if (existingRole !== "itdev" && !isTeacher) {
        sessionStorage.setItem("daeroUserRole", "admin");
    }

    const footer = document.querySelector(".admin-footer");
    if (footer) {
        const teacher = normalizeAssignedTeacher(sessionStorage.getItem("daeroAssignedTeacher"));
        const roleLabel = isTeacher && teacher
            ? `Teacher (${teacher})`
            : (existingRole === "itdev" ? "IT-Dev" : "Admin");
        footer.textContent = `Logged in as: ${roleLabel}`;
    }
...
function showAdminPage(pageId) {
    if (!canCurrentUserAccessTarget(pageId)) {
...
document.querySelectorAll('.admin-link').forEach(link => {
  link.addEventListener('click', async (ev) => {
    ...
    if (!target) return;
        if (!canCurrentUserAccessTarget(target)) {
            alert('You do not have permission to access this page.');
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!anyVisible) {
        const fallback = getSidebarLinkItemsForPermissions().find((item) => canCurrentUserAccessTarget(item.target));
        const firstAllowed = canCurrentUserAccessTarget('newApplicantsPage')
            ? 'newApplicantsPage'
            : (fallback ? fallback.target : 'programsPage');
        showAdminPage(firstAllowed);
  }
